...
__global__ 
void vector_reduction_kernel (float *A, double *result, unsigned int num_elements, int *mutex)
{
	__shared__ double sum_per_thread[THREAD_BLOCK_SIZE]; /* Allocated shared memory for the thread block. */
	unsigned int thread_id = blockIdx.x * blockDim.x + threadIdx.x; /* Obtain the index of the thread. */
	unsigned int stride = blockDim.x * gridDim.x; /* Stride for each thread. */
	double sum = 0.0f; 
