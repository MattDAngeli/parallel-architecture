/* Matrix vector multiplication b = Ax using omp. A is m x n matrix and x is a n x 1 vector.
 *
...
main (int argc, char **argv)
{
    ...
    if (argc < 5) {
        printf ("%s num-rows num-columns num-threads threading-threshold\n", argv[0]);
        printf ("num-rows, num-columns: Number of rows and columns in the A matrix\n");
        printf ("num-threads: Number of threads to create\n");
        printf ("threading-threshold: Threshold beyond which to use multi-threading\n");
        exit (EXIT_SUCCESS);
    }
    ...
    printf ("Vector matrix multiplication using omp. Version 1.\n");
    gettimeofday (&start, NULL);
    ...
    printf ("Vector matrix multiplication using omp. Version 2.\n");
    gettimeofday (&start, NULL);
...
/* Calculate the solution using omp. No bells and whistles. */
void 
compute_using_openmp_v1 (float *matrix_a, float *vector_x, float *vector_b, int num_rows, int num_cols, int thread_count, int threshold)
{
    int i, j;
...
/* Implement a thresholding scheme to decide whether to go multi-threaded or not. This code may exhibit false sharing since the 
 * partial sums are being constantly accumulated directly into the vector_b variable. */
void 
compute_using_openmp_v2 (float *matrix_a, float *vector_x, float *vector_b, int num_rows, int num_cols, int thread_count, int threshold)
{
    int i, j;
	omp_set_num_threads(thread_count);
...
/* Implement a thresholding scheme to decide whether to go multi-threaded or not. Also reduces the false sharing problem from the previous verion. */
void 
compute_using_openmp_v3(float *matrix_a, float *vector_x, float *vector_b, int num_rows, int num_cols, int thread_count, int threshold)
{
	int i, j;
	double sum;
